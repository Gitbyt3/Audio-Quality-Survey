...
import audio from '../audios/Real1.mp3'

const Real1 = () => {
    const { surveyData, updateSurveyData } = useSurvey()

    const initialData = {Score: 1, Comment: '', ...surveyData.Real1}

    const [ formData, setFormData ] = useState(initialData)
    const [isTouched, setIsTouched] = useState(!!surveyData.Real1.Score)
    ...
    return (
        <div style={{ padding: '20px', textAlign: 'center' }}>
            ...
            <PageNavigation onNavigate={() => updateSurveyData('Real1', formData)} nextDisabled={!isFormComplete()} />
        </div>
    )
}
