...
import audio from '../audios/RealScript_Adbuild1.mp3'

const RealScript_Adbuild1 = () => {
    const { surveyData, updateSurveyData } = useSurvey()

    const initialData = {Score: 1, Comment: '', ...surveyData.RealScript_Adbuild1}

    const [ formData, setFormData ] = useState(initialData)
    const [isTouched, setIsTouched] = useState(!!surveyData.RealScript_Adbuild1.Score)
    ...
    return (
        <div style={{ padding: '20px', textAlign: 'center' }}>
            ...
            <PageNavigation onNavigate={() => updateSurveyData('RealScript_Adbuild1', formData)} nextDisabled={!isFormComplete()} />
        </div>
    )
}

export default RealScript_Adbuild1